...
try:
    ...
    BASE_DIR = Path.cwd()
ART_DIR = BASE_DIR / "noisy_opt" / "artifacts"
RESULTS_DIR = BASE_DIR / "noisy_opt" / "checkpoints"
...
shared_model_path = BASE_DIR / "models" / "facebook/opt-1.3b"
...
def _client_dir(cid: int) -> str:
    d = os.path.join(ART_DIR, f"client_{cid}_20news")
...
if __name__ == "__main__":
    ...
    T = args.times
    # alpha = np.round(np.random.rand(num_clients).tolist(),2)
    # alpha = np.round(rng.random(num_clients), 2)
    # beta  = np.round((1.0 - np.asarray(alpha)).tolist(),2)
    
    # r     = rng.uniform(1.0, 10.0, size=num_clients)     
    # alpha = rng.uniform(r, 10.0, size=num_clients)    
    # beta  = alpha / r
